...
bool eliminar_primer_dato(nodo_listaSE* &listase){
    if (listase != NULL)
    {
...
void insertar_ordenado(nodo_listaSE* &listase, int dato_insertar){
    //supone que la lista esta ordenada en forma ascendente y agrega el dato
    nodo_listaSE* actual = listase;
    nodo_listaSE* anterior = NULL;
    nodo_listaSE* nuevo_dato = new nodo_listaSE;
    nuevo_dato->link = NULL;
    nuevo_dato->dato = dato_insertar;
    if (listase == NULL || listase->dato > dato_insertar) {
        nuevo_dato->link = listase;
        listase = nuevo_dato;
        
    }
    else{
        while (actual != NULL && actual->dato < dato_insertar)
        {
            anterior = actual;
            actual = actual->link;
        }
    }
    nuevo_dato->link = actual;
    anterior->link = nuevo_dato;
}
...
int main()
{
    ...
    do
    {
        ...
        case 6:
            ...
            break;
        case 7:
            cout << "Ingrese el dato que desea agregar ordenado: ";
            cin >> dato;
            insertar_ordenado(listase,dato);
            break;
        case 0:
